...
|----------|--------|----------|----------|----------|
| 代码逆向 | 代码分析、逻辑提取、算法还原 | Decoder | 单专家 | 可能需要 |
| 迁移规划 | 迁移策略、技术选型、路线规划 | Pathfinder | 链式 | 可能需要 |
| 架构设计 | 加载机制、资源管线、接口设计 | Bridge | 单专家 | 可能需要 |
| 代码复刻 | 代码重写、现代化改造、功能实现 | Mimic | 链式 | 可能需要 |
| 资产优化 | 贴图压缩、模型优化、格式转换 | Palette | 单专家 | 可能需要 |
| 资产盘点 | 目录扫描、去重、命名规范 | Vault | 单专家 | 通常不需要 |
| 全栈迁移 | 项目移植、引擎升级、平台迁移 | 多专家协作 | 并行 | 按阶段 |
